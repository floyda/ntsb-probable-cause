"""ntsb-ingest: fetch raw pages and build the processed file."""
...
from ntsb_probable_cause.data.build import SPIKE_SPLIT_COUNTS, build_processed
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    commands.add_parser("build", help="build data/processed/cases.parquet from the raw store")
    ...
    if args.command == "build":
        ...
        print(
            f"rows {result.rows}; replaced duplicates {result.duplicates_replaced}; "
            f"excluded {dict(result.excluded)}"
        )
        ...
        for split, spike in SPIKE_SPLIT_COUNTS.items():
            ...
            print(
                f"reconciliation {split}: this build {ours}, spike {spike}, "
                f"difference {ours - spike:+d}"
            )
        return 0
    with NtsbClient(
