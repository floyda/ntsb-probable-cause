# 0038 — The docket filter is measured by tripwire hits, and its type labels by a title hand-check
...
0037, which reduced the filter's job.

## Context

After 0037, every docket document is evidence and the filter has two jobs left:
...
2. **Sort documents into types** for the provenance header (0037) and for arm B's filter
...
4. **The sample is the docket listings of `dev-400`** (0026), fetched once. Listing pages and
   ...
   re-measurement and the fixture pool (0036).
