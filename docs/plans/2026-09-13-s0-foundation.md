...
- Task 15 (review fix round 1–2): Section 2 stop conditions were rewritten in round 1 to exempt close-out task steps from the "unticked steps → stop" check and add evidence recording for the close-out condition. Round 2 fixes: (1) **Important, reversed logic**: "no open pull request exists for the branch (gh pr view fails)" — the brief's wording said fail if PR does NOT exist, not if it does; (2) **Important, clarified exemption**: replaced false claim that steps "will be ticked once this skill completes" with precise logic — steps are exempted only "from the step that runs `/close-stage` onward", not all close-out task steps; steps before that point "must be ticked"; the plan is deleted so no step can be ticked after the skill runs; (3) added explicit evidence-writing guidance to Section 4 for the close-out condition and a confirmation step in Section 6; (4) added three specific string assertions to the test (`that runs `/close-stage` onward`, `no open pull request`, `exists for this branch`, `git branch --show-current`). The brief's original stop conditions would have halted the skill during Task 16 (unconditional stop on unticked steps in any close-out task, plus unreachable Done-means evidence). Task 16 opens the PR before running `/close-stage`, so round 1's reversed PR check would have failed on every real run. No decision record (fixes a plan bug in the rule the task itself implements, per decision 0017).
