...
- [x] **Step 5: Fetch three development months from the real API**
...
Expected: each prints `fetched 1 months, N records`; `data/raw/v2/manifest.jsonl` has three lines; `git status` shows nothing under `data/`. Then: `uv run python -c "from pathlib import Path; from ntsb_probable_cause.data.ingest import read_manifest, latest_entries, verify_entry; r=Path('data/raw'); l=latest_entries(read_manifest(r)); [verify_entry(r, e) for e in l.values()]; print('verified', len(l), sum(e.records for e in l.values()))"` prints `verified` with a month and record count.
...
### Task 18: Amateur-built make and model replaced in evidence (decision 0020)

*Added 2026-09-14 at Andy's decision, after the final whole-branch review; runs before Task 16's close-out commit. Not in the S0 specification — recorded under Deviations and in decision 0020.*

**Files:**
- Modify: `src/ntsb_probable_cause/fields.py` (make and model extractors, sources, label constant)
- Modify: `scripts/corpus_scan.py` (amateur-built counts), `docs/results/s0-corpus-scan.txt` (re-run)
- Modify: `CLAUDE.md` (rule 9, one sentence)
- Test: `tests/test_fields.py`, `tests/test_boundary.py`, `tests/test_corpus_scan.py`
...
- Task 9, step 2 (review fix round 1, important): a trivial edit defeated the tripwire — a needle kept its final `.`/`!`/`?`/`;`/closing quote or bracket, so a copy of withheld text with that mark dropped, appended-to, or with a smart apostrophe/quote substituted returned no leak, and a sentence boundary with no space after the mark (`"examined thoroughly.No anomalies"`) or delimited only by `;` was never split out as its own needle. Fixed in `records/guard.py`: `normalise_text` now applies `unicodedata.normalize("NFKC", ...)` and folds curly single/double quotes (`‘’‚‛` / `“”„‟`) to straight ones before collapsing whitespace and lower-casing; a new `_strip_trailing_punct` (regex `[.!?;:'")\]}]+$`) strips trailing sentence punctuation and closing quotes/brackets from every text and sentence needle (not the haystack); `_SENTENCE_END` now also splits on `;` and on `.`/`!`/`?` directly followed by a letter with no space; the `sentence != whole` de-duplication compares the two after stripping. `MIN_SENTENCE_CHARS` unchanged (Task 13 measures it). Added 8 targeted tests (one per reviewer probe) and a second hypothesis property, `test_any_withheld_text_without_its_final_punctuation_is_still_found`, appending a random final-punctuation character to the withheld text while the evidence copy carries none. All ten new/changed guard tests were confirmed to fail against the pre-fix code (RED) before the fix, and pass after (GREEN). No decision record (bug fix in the plan's own code, ruled Important by review).
- Task 9, step 2 (review fix round 1, minor): `Leak.__str__` and `split_record`'s `LeakageError` message included up to 80 characters of the withheld fragment itself — from S3 an error message could reach model context or the board. `Leak.fragment` is kept (for callers needing the structured detail) but `__str__` now renders only `{kind} from {source} in {evidence_role} ({len(fragment)} chars withheld)`, never the text. `errors.LeakageError` gained a keyword-only `leaks: Sequence[object] = ()` constructor parameter and a `self.leaks` attribute (typed `Sequence[object]`, not `Sequence[Leak]`, so this foundational module never depends on `records.guard`; class name and the rest of the hierarchy unchanged). `split_record` now passes `leaks=leaks` when raising. Added `test_leak_str_does_not_render_the_fragment_text` and extended `test_tripwire_fires_when_a_record_carries_withheld_text_in_evidence` to assert the raised message does not contain the fixture's actual probable-cause text and that `exc_info.value.leaks` is non-empty; both confirmed RED against the pre-fix code. No decision record (bug fix, ruled Minor-include by review).
- Task 9, step 2 (review fix round 1, minor): `_BOILERPLATE` matched any sentence merely starting with "this report was modified on", so a probable cause prefixed with that phrase would have been wrongly exempted, and it was applied to the whole-text needle as well as each sentence. Verified the real shape against development records containing the phrase (`data/raw/v2/2014-06`, `data/raw/v2/2016-08`, read-only): `"**This report was modified on <date>. Please see the (public) docket for this accident to view the original report.**"` or, without the second sentence, `"**This report was modified on <date>.**"`, `<date>` as `Month D, YYYY` or `M/D/YYYY`, wrapped in `**`/`***` and/or quotes. Renamed to `_BOILERPLATE_SENTENCE` and anchored to the full sentence only: `^\W*this report was modified on (?:[a-z]+ \d{1,2}, \d{4}|\d{1,2}/\d{1,2}/\d{4})\W*$` (the brief's example `[\d/]+` covers only the numeric-date form; both real forms are accepted). Removed the boilerplate check from the whole-text needle branch entirely — it is never exempted, per the review's instruction. Renamed test `test_boilerplate_is_ignored` to `test_bare_boilerplate_sentence_alone_is_not_flagged` (unchanged behaviour: the boilerplate sentence alone in evidence is not a leak) and added `test_probable_cause_prefixed_before_boilerplate_is_still_caught` (the actual cause text in evidence, with the verdict field carrying cause-then-boilerplate, is still caught). No decision record (bug fix, ruled Minor-include by review).
...
- Task 10, additional work item 3 (review fix round 1, important): the hand-built-Evidence mutation test failed on provenance in every case, never the tripwire — `match="provenance|tripwire"` could not tell the two apart, and deleting the tripwire block in `tests/boundary.py` left the suite green. Replaced it with `test_boundary_fails_when_only_the_tripwire_can_catch_a_leak` in `tests/test_boundary.py`: deep-copies a fixture, sets `aircrafts[0].aircraftMake` to the record's own `probable_cause` text (so the real extractor's provenance check holds — the sent value matches the raw record exactly), and monkeypatches `ntsb_probable_cause.records.split.find_leaks` to `lambda *a, **k: []` so `split_record`'s own guard does not raise before the boundary check runs; asserts `assert_boundary_holds` raises `AssertionError` matching `^tripwire`. Verified by temporarily deleting the tripwire block (the `withheld = {...}` / `find_leaks` / final `assert not leaks` lines) from `tests/boundary.py`: the new test failed with `Failed: DID NOT RAISE AssertionError`; restored the block from a backup, confirmed `git diff` showed only the intended docket-URL change (item 4 below), and re-ran to confirm it passes again. Also tightened `test_boundary_test_fails_when_the_splitter_leaks` and `test_boundary_test_fails_when_a_value_comes_from_the_wrong_place` to `match=r"^provenance"`, naming the layer each actually proves. No decision record (bug fix and test-coverage gap, ruled Important by review).
- Task 10, step 2 (review fix round 1, minor): `Payload` could be mutated after construction (`payload._text = ...`) and subclassed to bypass the construction token. Added `@typing.final` to the class; `__init__` now sets the slot via `object.__setattr__(self, "_text", text)`; `__setattr__` and `__delattr__` are overridden to always raise `AttributeError`. Added a class-level `_text: str` annotation (no assignment) alongside `__slots__` so mypy still knows the attribute's type through the `object.__setattr__` indirection. Added `test_payload_is_immutable_after_construction` to `tests/test_model_client.py`, asserting both an attribute set and a `del` raise `AttributeError`. No decision record (hardening, ruled Minor-include by review); no public name or signature changed.
- Task 10, step 2 (review fix round 1, minor): `Payload.from_evidence`'s render-time `LeakageError` always reported `sorted(keys - _EVIDENCE_NAMES)`, which is empty (prints `[]`) in the realistic mutation where a withheld role name collides with an existing evidence role name (`keys & WITHHELD_ROLE_NAMES` nonempty, `keys <= _EVIDENCE_NAMES` still true) — the offending key was never named. Changed to report `sorted((keys - _EVIDENCE_NAMES) | (keys & WITHHELD_ROLE_NAMES))`. Added `test_render_check_error_names_the_offending_role` to `tests/test_model_client.py`, which monkeypatches `ntsb_probable_cause.model.client.WITHHELD_ROLE_NAMES` to collide with the fixture's `aircraft_make` role and asserts the raised `LeakageError` names it. No decision record (bug fix, ruled Minor-include by review); no public name or signature changed.
...
- Task 8/9 (final review, item A, important): `scripts/make_fixture.py`'s `_narrative_texts` read a top-level `record["probableCause"]`, which does not exist on a real record (the real path is `narratives[].probableCause`), and never read `narratives[].prelimNarrative` — both are real free-text fields checked against `tests/fixtures/records/*.json`. Changed to collect `concatenatedFactualNarrative`, `analysisNarrative`, `probableCause` and `prelimNarrative` from every `narratives[]` entry. Also, the `records <id>` subcommand built its record dict directly from `iter_raw_records` and wrote it without ever calling `_eligible()` or `_amateur_built_name_risk()`, so `make_fixture records <id>` could write a held-out-eligible-looking but actually ineligible (e.g. not completed) or name-risked record straight to `tests/fixtures/records/`; it now raises `FixtureError` naming the id and the reason for either case, before writing. Added `test_make_naming_the_builder_only_in_probable_cause_is_a_name_risk`, `test_make_naming_the_builder_only_in_prelim_narrative_is_a_name_risk`, `test_records_command_refuses_an_ineligible_record` and `test_records_command_refuses_a_screened_record` to `tests/test_fixtures.py` (the latter two via a fake `CasesSource` and `fetch_months` into a `tmp_path` raw store, monkeypatching the script's module-level `RAW`/`RECORDS` constants); all four confirmed to fail on the pre-fix code (RED) and pass after (GREEN). Confirmed read-only that all nine committed record fixtures and the three API page fixture records still pass every fixture test (`tests/test_fixtures.py`, 12 tests) without regeneration. No decision record (bug fix, ruled Important by the final review); no public name or signature changed.
- Task 8 (final review, item B, important): `build_processed`'s `duplicates_replaced` counted every case number seen more than once across month partitions, not only the cases where a later record actually displaced the kept one — a stale entry encountered after the newest copy was already kept (possible because `iter_raw_records` yields month-label order, not `fetched_at` order) was still counted as a replacement. Split the counter into `seen` (every repeat occurrence) and `replaced` (only when `entry.fetched_at` exceeds the kept copy's, i.e. an actual displacement); `BuildResult` gained a new field `duplicates_seen: int` alongside the existing `duplicates_replaced: int` (both flow into `cases.meta.json` via the existing `asdict(result)`). Added `test_build_deduplicates_across_month_partitions_by_fetched_at` to `tests/test_build.py` with three cases: (A) a later month partition with a later `fetched_at` carries `completionStatus="Ongoing"` — the newer copy wins before filtering (excluded as "not completed") and is counted as exactly one replacement; (B) a later partition, Completed, with a changed field — the newer value wins and is counted as one replacement; (C) constructed so a *stale* entry (older `fetched_at`) is iterated *after* the newest is already kept (an earlier month label, "2016-07", fetched later than a subsequent label, "2016-08") — confirmed to fail on the pre-fix code (`duplicates_replaced == 1`, expected `0`; RED) because the old counter could not distinguish a repeat that was never applied from an actual displacement, and to pass after the fix (`duplicates_replaced == 0`, `duplicates_seen == 1`; GREEN), with the kept row's value confirmed to be the true newest (`"Fatal"`, not the stale `"Minor"`). No decision record (bug fix, ruled Important by the final review); `BuildResult`'s only change is the additive `duplicates_seen` field, `duplicates_replaced`'s name and meaning ("actual replacement") are unchanged from the field's own docstring intent.
- Final review, item C: `README.md` and `CLAUDE.md` still described S0 as not built, with no
  commands to run — both are now stale, since S0's code, tests and scripts exist. `README.md`'s
  status line now says S0 is built (foundation only; no agent), its layout section lists
  `src/ntsb_probable_cause/`, `apps/`, `scripts/`, `tests/`, `docs/plans/`, `docs/results/`, and
  it gained a "Commands" section naming the real Makefile targets, `scripts.make_fixture` and
  `scripts.check_docs`, and the `NTSB_API_KEY`/`NTSB_DATA_DIR` settings. `CLAUDE.md`'s intro
  sentence "No code exists here yet" is replaced with a one-line S0-built summary; rule 9's
  toolchain list gained `vulture` (already run by `make lint`/pre-commit, previously unlisted);
  the final "Note" section (which said to add a "how to run" section only once code exists) is
  replaced by the same "Commands" content as `README.md`. `uv run python -m scripts.check_docs`
...
- Task 18 (Task 18 review): `tests/test_boundary.py::test_boundary_holds_and_hides_the_builder_name_for_amateur_built_aircraft` set only an invented `aircraftMake`, so an invented `aircraftModel` reaching the payload undetected would not have been caught. Added an invented `aircraftModel` ("INVENTED MODEL") alongside the existing invented make, and asserted it too is absent from `Payload.text`. No decision record (test-coverage gap, not a change of approach).
- Task 18 (Task 18 review): `fields._amateur_built_or_text_at`'s flag check was fail-open for a non-boolean flag — it used the recorded make/model unless the flag was exactly `True`, so a truthy-looking but non-boolean value (for example the string `"true"`) fell through to the real, unredacted value instead of the label. Changed the rule to fail closed: the label is used unless the flag `is False` or is absent/`None`; any other value, including a non-boolean one, gives the label. Updated the `AMATEUR_BUILT_LABEL` comment and `_amateur_built_or_text_at`'s docstring to state the rule, and added `test_non_boolean_amateur_built_flag_still_gives_the_label` (parametrised on `"true"` and `1`) to `tests/test_fields.py`. Every real value in the processed corpus is a JSON boolean, so the corpus scan's own counts are unaffected. Aligned `scripts/corpus_scan.py`'s `is_amateur_built` to the same fail-closed rule (it previously used `is True`, the same narrower check `fields.py` had) and added `test_is_amateur_built_fails_closed_for_a_non_boolean_flag` to `tests/test_corpus_scan.py`; re-ran `uv run python -m scripts.corpus_scan` and confirmed its output is byte-for-byte unchanged (real data holds only booleans), so `docs/results/s0-corpus-scan.txt` was not recommitted. No decision record (bug fix, ruled Important by Task 18's review); no public name or signature changed.
