...
- 2026-09-18, Task 7 fix round 1, Finding 1: `tests/test_docket_listing.py`'s `_saved_pages()`-derived tests read the fixture with `Path.read_text()`, which is text-mode universal-newlines and turns the fixture's CRLF into LF before `parse_listing` ever sees it -- while `DocketClient.listing_html` decodes raw response bytes and never does that translation. So the suite was not testing the text the parser meets in production, which is exactly what the byte-exact fixture (decision 0037) exists to prevent. Fixed with a `_read_page()` helper (`page.read_bytes().decode("utf-8")`) used by both tests, plus a guard assertion (`"\r\n" in text`) so a future LF-only fixture would fail loudly rather than the CRLF path going untested in silence. `_ROW`, `_ITEMS` and `parse_listing` needed no change: `\s` already covers `\r` and `\n`.
- 2026-09-18, Task 7 fix round 1, Finding 2: nothing in the repository locked the docket fixtures' line endings on checkout, so byte-exactness rested on every clone's `core.autocrlf` setting rather than being enforced. Added `.gitattributes` at the repository root: `tests/fixtures/docket/** -text`. Confirmed with `git check-attr text tests/fixtures/docket/ERA17LA217/listing.html` -> `text: unset`.
...
- 2026-09-18, Task 12: `_cmd_run` builds `CachedDocketReader(DocketClient(...))` only for `--arm B`; used `contextlib.nullcontext()` for every other arm so one `with` block covers both cases (the brief said "a `with` block around the run" without specifying how to make it conditional). No test exercises the `--arm B` path through `main()` -- it would need a real (or extensively faked) `DocketClient`/HTTP layer, and the coverage gate (`--cov=ntsb_probable_cause`) does not measure `apps/`, so this is a gap worth flagging rather than one `make check` catches. (Closed in fix round 1, Finding 3, below.)
- 2026-09-18, Task 12 fix round 1, Finding 1: the leakage guard was pinned only at the level `tests/test_attach.py` calls it (`split_record` directly on an already-attached context), which does not prove `prepare_case` -- the actual arm B production path -- runs the same guard. Added `test_prepare_case_fails_closed_when_an_attached_document_holds_the_narrative` to `tests/test_runner.py`: drives `prepare_case` itself with an arm B spec and a docket document holding a real fixture's factual narrative (read at run time via `factual_narrative(raw)`, as `test_attach.py` does; never written to a file), and asserts `LeakageError` comes out of `prepare_case`. No production code changed -- the ordering was already correct, only unproven at this level.
...
- 2026-09-18, Task 13 fix round 1, finding 1: a case skipped for a missing `mKey`, or whose listing failed after every retry, was reported only on stderr -- `docs/results/s2-shape-dev.txt` would look identical whether the scan covered 400 dockets or 385, over a silently different population. Added three counters to `ShapeState` (`cases_attempted`, `listing_failed`, `missing_mkey`) and three small recorder functions (`record_attempt`, `record_missing_mkey`, `record_listing_failed`), called once per case in `main`'s loop before the outcome is known (`record_attempt` for every case; then exactly one of `record_missing_mkey`, `record_listing_failed`, or `accumulate` -- the pre-existing counter that already meant "docket successfully read"). `report()` prints one new line per stratum group, right after the `## {name}: {n} dockets` header: `cases attempted: A; listing fetch failed: L; skipped for missing mKey: M`, so `A - L - M` should equal the header's own `n` for a complete, honest run. Refactored the repeated `"fatal" if fatal else "non-fatal"` into a `_stratum(fatal)` helper, used by both `accumulate` and the three new recorders, and computed `fatal` once per loop iteration in `main` instead of twice. Added `test_report_states_the_denominator_a_reader_needs_to_trust_the_numbers`.
- 2026-09-18, Task 13 fix round 1, finding 2: four figures in the report read as one measurement when they are not. Added a "Definitions and limits" section, printed once up front (before the per-stratum breakdown, since it is meta information the whole report shares): the owner/operator name figures are counted only over documents whose text was extracted (a scanned document is never searched -- a floor for a second reason, beyond names the record does not hold); "containing" is a plain substring test while the replacement counts are word-boundary anchored (decision 0046), so the two are not the same measurement; tokens per docket sums each document's own characters-divided-by-four floor, which the spike's own figures did not do (the spike floored once over a docket's total characters); and "scanned pages" counts every page of a document classified as a scan, because readable pages are only computed for a document that was read, not one classified as a scan up front. Added `test_report_states_definitions_and_limits`.
- 2026-09-18, Task 13 fix round 1, finding 3: `check_fixtures_redacted.py`'s docket check only ever looked at a case folder's immediate children, so a document committed directly at `tests/fixtures/docket/` or nested one level deeper inside a case folder went unchecked, and a case folder with no `manifest.json` raised `FileNotFoundError` instead of reporting a problem. Rewrote `docket_fixture_problems` around `root.rglob("manifest.json")` (which only ever returns files that exist, so there is nothing left to raise) and `root.rglob("*.pdf")`/`root.rglob("*.txt")`: every manifest is read once, building a map from its directory to the absolute paths it lists (resolved as `case_dir / name`, so a nested `"sub/2.pdf"` entry works the same as a top-level one); every committed document file is then matched to the nearest manifest directory at or above it (`_nearest_manifest_dir`, walking upward, never past `root`) and flagged as `not listed` if that manifest doesn't name it, or as having `no manifest.json` above it at all if the walk reaches `root` without finding one. Added three tests: `test_a_file_directly_under_the_fixtures_root_is_a_problem`, `test_a_file_in_a_nested_subfolder_not_listed_is_a_problem`, `test_a_case_folder_without_a_manifest_is_a_problem_not_a_crash`. The three original Step-1 tests (reviewed-by missing, orphan file, clean case) needed no change: the new algorithm produces the same result for a document sitting directly in a case folder that has its own manifest.
- 2026-09-18, Task 13 fix round 1, finding 4: `quantiles` documented "nearest-rank" but used `round(q * n)`, which is half-to-even and returns one rank low at a half-integer rank with an even floor (n=5 at p90: rank 4, not 5; n=150 at p75: rank 112, not 113) -- exact at n=400, so the pre-existing test and the report's overall row were unaffected, but a fatal/non-fatal stratum row (never exactly 400) would have been off by one rank. Switched to `math.ceil(q * n)`, which matches the docstring and agrees with `round` at every non-half-integer rank -- the existing `test_quantiles_are_nearest_rank` (n=4, no half-integer ranks at the tested quantiles) still passes unchanged. Added `test_quantiles_round_up_at_a_half_integer_rank`, pinning both cases named in the finding.
